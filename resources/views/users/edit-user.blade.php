...
<body>
    <div class="container mt-5" style="width: 1000px;">
        ...
        @endif
        <form action="{{route('users.editPutUsers', $user->id)}}" method="POST">
            @csrf
            @method('PUT')
            <div class="mb-3">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="age" name="age" value="{{$user->tuoi}}">
                @error('age')
                  <div class="form-text text-danger">{{$message}}</div>
              @enderror
              </div>
            <div class="mb-3">
                ...
                <select class="form-select" name="phongban_id">
                    ...
                    @foreach ($phongBans as $phongBan)
                        <option value="{{$phongBan->id}}" {{$phongBan->id == $user->phongban_id ? 'selected' : ''}}>{{$phongBan->ten_donvi}}</option>
                    @endforeach
                </select>
                @error('phongban_id')
                  <div class="form-text text-danger">{{$message}}</div>
